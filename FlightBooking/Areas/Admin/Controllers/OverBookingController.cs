...
namespace FlightBooking.Areas.Admin.Controllers
{
    ...
    public class OverBookingController : Controller
    {
        ...
        public OverBookingController(NoShowService noShowService, OverbookingRecommendationService overbookingRecommendationService)
        {
        ...
        {
            var flights = await _noShowService.GetAllAsync();
            ...
            foreach (var flight in flights)
            {
                var recommendation = await _overbookingRecommendationService.GenerateRecommendationAsync(
                    ...
                    flightSlot: flight.FlightSlot,
                    passengerCount: flight.BoardedPassenger,
            ...
            var dto = new OverbookingDashboardDto
            {
                ...
                AverageNoShowRate = recommendations.Average(x => x.ExpectedNoShowRate),

                MostRiskySlot = recommendations.OrderByDescending(x => x.ExpectedNoShowRate).First().FlightSlot,

                MostStableSlot = recommendations.OrderBy(x => x.ExpectedNoShowRate).First().FlightSlot,

                SuggestedOverbookingRate = recommendations.Average(x => x.ExtraSellableSeatCount),

                TotalFlightCount = recommendations.Count,

                TotalPassengerCount = recommendations.Sum(x => x.ActualPassengerCount),
